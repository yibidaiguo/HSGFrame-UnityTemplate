...
function 渲染门禁(数据) {
    ...
    var 文本 = '<div class="格 自适应" style="margin-bottom:16px;">' +
        ...
        '</div>';
    if (状态 === '未跑') {
        // 这里原来写的是「跑一次 gate.ps1 再回来看」——那是句假话：这份报告至今没有生产者，
        // gate.ps1 只往控制台打、不落文件（P1 批次 5 的已知缺口，至今没销）。
        // 于是这一页在真仓库里恒为「未跑」，跟门禁跑没跑过、绿不绿都无关。
        // 教人做一件做了也没用的事，比不说更糟。
        文本 += 提示态('这一页现在拿不到数据，而且跑门禁也不会让它有',
            '要读的报告路径是 ' + (数据['报告路径'] || '（接口没给路径）') +
            '，但这份报告至今没有生产者——gate.ps1 只往控制台打，不落文件。' +
            '所以「未跑」在这里不代表你没跑过门禁，只代表这个文件不存在。' +
            '这一页要真能用，得先有人把门禁结果落成那份 JSON。');
    }
    if (条目.length > 0) {
