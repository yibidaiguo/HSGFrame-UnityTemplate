...
namespace Template.Toolkit.Editor
{
    ...
    public static class YooAssetSimulateBootstrap
    {
        ...
        /// <summary>成品资源单元的采集根，相对 Unity 工程写。</summary>
        public const string LevelResourceCollectRoot = "Assets/Game/ResourceArt/Level";
        ...
        /// <summary>
        /// 把采集规则配成《结构规范-资源》第二、三节的目标态：收集入口只有 ResourceArt/ 与 Scenes/World/，
        /// Art/ 作为依赖被拖进包、不直接收集。已经配过就原地更新。
        /// </summary>
        ...
        public static void ConfigureCollector()
        {
            ...
            // 调用方得写 "Assets/Game/ResourceArt/Level/P_Npc.prefab" 才取得到。
            package.EnableAddressable = true;
            package.Groups.Clear();

            package.Groups.Add(BuildGroup(
                LevelResourceGroupName,
                "关卡实体预制体，按需加载",
                LevelResourceCollectRoot));
            package.Groups.Add(BuildGroup(
                WorldSceneGroupName,
                "热更玩法场景，随场景加载",
                WorldSceneCollectRoot));

            BundleCollectorSettingData.SaveFile();
            Debug.Log($"[资源] 采集规则已配好：包裹 {PackageName}，" +
                      $"采集根 {LevelResourceCollectRoot} 与 {WorldSceneCollectRoot}");
        }

        // 一个组一条收集路径：组名与收集路径都要能和打包分组规则里的条目一一对上，
        // 一个组塞多条路径就对不了账了。
        private static BundleCollectorGroup BuildGroup(string groupName, string groupDesc, string collectRoot)
        {
