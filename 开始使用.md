...
```bash
git clone <本仓库> GameTemplateForAgent
cd GameTemplateForAgent
```
...
**UPM 包不用管**：HybridCLR 与 YooAsset 都写在 `UnityProject/Packages/manifest.json` 里，
Unity 打开工程时自己会去拉。

**要装一次的只有一样**：HybridCLR 的本地 il2cpp 数据（约 800 MB）。
...
| 想手动装 | 菜单 **GameTemplateForAgent → 初始化开发环境** |
| 无人值守 / CI | `./Tools/Cli/unity-cmd.ps1 -ExecuteMethod Template.Toolkit.Editor.EnvironmentInstaller.InstallFromCommandLine -TimeoutMinutes 40` |
...
```powershell
./Tools/Gates/gate.ps1          # 六道门禁：测试 / 编译 / 命名 / 基线 / 白名单 / 文档，约 30 秒
