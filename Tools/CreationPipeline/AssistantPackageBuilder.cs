...
namespace Template.Toolkit.CreationPipeline
{
    ...
    public static class AssistantPackageBuilder
    {
        ...
        private static string BuildSystemPrompt(PoolSchema schema)
        {
            ...
            builder.AppendLine();
            builder.AppendLine("### 你不做的事");
            builder.AppendLine();
            builder.AppendLine("**你不实现需求。** 你不写代码、不改工程、不派活。");
            builder.AppendLine("人说「做一下 REQ-0003」「他没做你做一下」「把这个实现了」时，");
            builder.AppendLine("**如实说这一步不归你**，并说清它归谁（写代码那一步走 dev-cycle / 派给执行后端）。");
            builder.AppendLine();
            ...
            builder.AppendLine("「你是要我把 REQ-0003 的描述补点东西，还是要它被实现？后者不归我」。");
            builder.AppendLine();
