...
## 1. 文件名与目录名去中文化（只保留注释与数据里的中文）—— **已完成**

**提出人**：用户，2026-08-21。
**状态**：**已完成，2026-08-21**。设计审查见
[路径去中文化](ascii-path-migration.md)，逐批落地见
[各批落地记录](ascii-path-migration-log.md)。
决策 1 已按审查结论改写；新增门禁 `gate.pathascii`，**存量 0 条、模式 block**，
反向验证过（造一个中文名当场判红）。
**下面这些是当时的分析，留着当账。**
...
**全仓的目录名与文件名一律 ASCII**，由 `gate.pathascii` 在 block 模式下把关。
中文照旧留在**文件内容**里：注释、文案、数据值、JSON 的键都不受限。

**还剩一条要用户点头**：资产名归一化器（`AssetInboxArchiver`）仍然把带中文的
图片名归一成中文名，且有测试锁住那个行为，与新门禁打架。
要么归一化器也转 ASCII（得有一套音译或翻译规则），要么给它开豁免——
两条都是策略决定，见 [e 批记录](ascii-path-migration-logs/e-unity-and-runtime.md) 末节。
...
## 2. `Config/` 分层：工具配置归位（与第 1 条合批做的）—— **已完成**

**提出人**：用户，2026-08-21。
**状态**：**已完成，2026-08-21**（c1、c3 两批）。
`Config/创作管线/` → `Tools/CreationPipeline/Config/`、`Config/Luban/` → `Tools/Luban/Config/`；
`Config/` 现在只剩 `Tables/` `Schema/` `Mirror/` 三样业务数据。决策 2 已改写。
**最危险那一步（`.gitignore` 里的密钥路径）验过**：
`git check-ignore` 命中新路径，`git status` 里没有 `local.json`。
**下面这些是当时的分析，留着当账。**
