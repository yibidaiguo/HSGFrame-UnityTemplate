...
function 渲染任务图(忽略, 完成) {
    ...
    拉('/api/panel/dag?requirement=' + encodeURIComponent(需求id)).then(function (数据) {
        if (!数据 || 数据.length === 0) {
            ...
            内容区.innerHTML = 头 + 卡在哪一步(数据) + 画依赖图(数据) + 任务图表格(数据);
        }
        绑需求框();
        应用过滤();
        if (完成) { 完成(); }
...
// 「卡在哪一步」：把失败与挂起的工作项挑出来摆在图上面。
//
// 图能看出结构，但**看不出轻重**——一张几十个节点的图里，一个红框要人一个个扫过去才找得到。
// 这一条横幅回答的是人打开这一页真正想问的那句话：现在卡在哪。
// 没有失败也没有挂起时**什么都不显示**，不摆一条「一切正常」的绿条占地方。
function 卡在哪一步(行列表) {
    var 失败 = [];
    var 挂起 = [];
    var i;
    for (i = 0; i < 行列表.length; i++) {
        var 状态 = 行列表[i]['状态'] || '';
        if (状态.indexOf('失败') >= 0) { 失败.push(行列表[i]); }
        else if (状态.indexOf('挂起') >= 0) { 挂起.push(行列表[i]); }
    }
    if (失败.length === 0 && 挂起.length === 0) { return ''; }

    var 条 = '';
    if (失败.length > 0) {
        条 += '<div class="板" style="border-color:var(--红);">' +
            '<div class="板题" style="color:var(--红);">卡住了 · ' + 失败.length + ' 个工作项失败</div>' +
            '<div style="padding:10px 14px;">';
        for (i = 0; i < 失败.length; i++) {
            条 += '<div style="margin-bottom:6px;"><span class="等宽" style="color:var(--红);font-weight:600;">' +
                转义(失败[i]['id']) + '</span>　' + 转义(失败[i]['标题'] || '') +
                '<span class="弱 小字">　第 ' + 转义(失败[i]['深度']) + ' 层</span></div>';
        }
        条 += '<div class="弱 小字">失败的工作项挡着它下游的所有节点——先看这几个。</div></div></div>';
    }
    if (挂起.length > 0) {
        条 += '<div class="板" style="border-color:var(--黄);">' +
            '<div class="板题" style="color:var(--黄);">挂起 · ' + 挂起.length + ' 个</div>' +
            '<div style="padding:10px 14px;">';
        for (i = 0; i < 挂起.length; i++) {
            条 += '<div style="margin-bottom:6px;"><span class="等宽">' + 转义(挂起[i]['id']) + '</span>　' +
                转义(挂起[i]['标题'] || '') + '</div>';
        }
        条 += '<div class="弱 小字">挂起是人按下的，不是出错——要它继续得有人来拨。</div></div></div>';
    }
    return 条;
}
...
// 工作项状态 → 节点样式类。状态取值是 标脏 / 挂起 / 执行中 / 已完成 / 失败（见 ReplanLanding）。
//
// **「失败」这一档以前没有**，于是一个失败的工作项画出来跟一个还没开始的一模一样——
// 而流程图最该回答的问题恰恰是「卡在哪一步」。
// 顺带修一个哑巴 bug：以前判的是 indexOf('进行')，而真实状态写的是「执行中」，
// 所以「做」这个类从来没生效过——在跑的节点一直显示成没开始。
function 节点类(状态, 深度) {
    if (深度 === -1) { return '环'; }
    var 文 = 状态 || '';
    if (文.indexOf('失败') >= 0) { return '错'; }
    if (文.indexOf('挂起') >= 0) { return '挂'; }
    if (文.indexOf('标脏') >= 0) { return '脏'; }
    if (文.indexOf('完') >= 0) { return '完'; }
    if (文.indexOf('执行') >= 0 || 文.indexOf('进行') >= 0) { return '做'; }
    return '';
}
...
function 画节点(行, 左, 顶, 宽, 高) {
    ...
    // 失败的节点在编号前加一个「!」并把编号标红：光靠底色在缩得很小的图上看不出来。
    var 前缀 = 类 === '错' ? '! ' : '';
    var 标类 = 类 === '错' ? '节点标 错字' : '节点标';
    return '<g><title>' + 转义(行['id'] + ' · ' + (行['标题'] || '') + ' · ' + 状态) + '</title>' +
        '<rect class="节点框 ' + 类 + '" x="' + 左 + '" y="' + 顶 + '" width="' + 宽 + '" height="' + 高 + '" rx="7"/>' +
        '<text class="' + 标类 + '" x="' + (左 + 10) + '" y="' + (顶 + 17) + '">' + 转义(前缀 + 编号) + '</text>' +
