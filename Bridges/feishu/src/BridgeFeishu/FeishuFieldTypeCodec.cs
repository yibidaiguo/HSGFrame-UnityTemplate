...
namespace Template.Bridges.Feishu
{
    ...
    ///
    /// **五个码全部实证过**（2026-08-21，P8 批次 13）：建一张临时探测表把五种类型各建一列、
    /// 读回 field 的 type 逐个核对、再把整张表删掉，五个码一个不差：
    /// 文本 1 / 数字 2 / 单选 3 / 多选 4 / 复选框 7。
    /// 在这之前 `数字=2` 与 `多选=4` 一直是**没被真回包验证过**的（P8 批次 7 记的缺口）——
    /// 因为需求表的 schema 里刚好一个数字列、一个多选列都没有。
    /// </summary>
