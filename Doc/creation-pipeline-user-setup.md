...
「自动」没人指定时，先挑**这台机器上次用它真跑成功过、且还在清单里的那个**；没有这条记账
才退到清单序数序第一项。为什么不按名字猜档位：那等于把模型名写死进代码。
**中转的 `/models` 常混着别的域的模型**（生图地址上列着一堆对话模型），
所以第一次用先带一次 `--Model` 指名，成功一次之后「自动」就跟着它走。每次调用都把
