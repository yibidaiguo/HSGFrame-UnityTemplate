...
- `dependencies.json`：两条（Impact-Pack 节点 + SDXL 底模）。**大文件刻意不进仓库，清单进**。
- `recipes/icon@v5/`：`workflow.json`（五节点骨架）+ `mapping.json`（五条字段映射 + 一个锚点槽 + 依赖引用）。
...
- **版本不比对。** 见决策 31。
- **配方与资产类型的对应关系没有门禁。** `mapping.json` 里有 `资产类型` 字段，
