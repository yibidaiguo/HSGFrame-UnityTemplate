...
- **面板开着不等于机器人活着**：机器人收消息靠长连接旁路，旁路没连上时飞书那头的消息
  **当场就丢，事后重启也补不回来**（旁路只收实时事件，不补历史）。踩过一次：三条消息全丢。
  现在 `panel.bat` 会把助手一起起，但**看日志确认**比看面板可靠——`Logs/assistant/serve.out.log`
  起来第一行就是「助手常驻会话起来了：…」，没有这行就是没起。
- **两个仓库不许连同一个飞书应用**：仓库内那把单实例锁跨不了仓库，两边都抢得到，
  于是同一条消息被收两遍、需求建两遍（REQ-0003/0004 就是这么来的）。
  现在有一把按 `应用标识` 分档的应用级锁挡着（`%LOCALAPPDATA%/HSGFrameRun/sidecar-locks/`）。
  **想两个项目同时开着，就给它们各建一个飞书应用**——应用不同，锁自然不同档。
