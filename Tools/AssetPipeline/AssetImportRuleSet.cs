...
namespace Template.Toolkit.AssetPipeline
{
    ...
    {
        /// <summary>从目录开始逐级向上找「导入规则.json」，找到即返回；找到文件系统根仍未找到则返回 null。</summary>
        ...
        /// <summary>从目录开始逐级向上找「导入规则.json」，找到即返回；到扫描根（含自身）仍未找到则返回 null。</summary>
        /// <param name="directoryPath">起始目录。</param>
        ...
        public static AssetImportRule LoadForDirectory(string directoryPath, string scanRoot)
        {
            ...
            while (true)
            {
                var candidatePath = Path.Combine(currentDirectory, "导入规则.json");
