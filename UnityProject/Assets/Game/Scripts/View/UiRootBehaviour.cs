...
namespace Template.View
{
    ...
    public sealed class UiRootBehaviour : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            {
                LoggerHub.Shared.Warning("位置：UiRootBehaviour；原因：场景里已经有一个 UI 根了，本个自行销毁；修复：UI 根跨场景常驻，只在启动场景里放一个；参考：Assets/Game/Scenes/Boot/启动.unity");
                Destroy(gameObject);
                return;
            }
            ...
            {
                LoggerHub.Shared.Error("位置：UiRootBehaviour；原因：UIDocument 上没挂 PanelSettings，视觉树建不起来；修复：把 Assets/Game/Settings/Ui/主面板设置.asset 挂到 UIDocument 上；参考：《结构规范-资源》第五节");
                return;
            }
