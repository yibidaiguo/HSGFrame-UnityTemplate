...
> 宿主项目的现状差距与迁移计划放宿主自己的文档区（本模板的开发宿主放在仓库根 `Doc/规范/`）。
...
| 5 | 资源热更新 | 动态区=收集根（`ResourceArt/`、`Scenes/World/`）；热更程序集清单单一源；AOT 不引热更 |
...
| 业务 | `UnityProject/Assets/Game/`（迁移期仍名 `_Project/`） | 项目的玩法与内容 | `Template.<模块>.*`，生成新项目时由 `project.create` 整体替换为项目名 |
...
2. **万物只做一份（单一事实源）。** 改一处，处处生效；复制粘贴出第二份就是缺陷：
   ...
   - 加载分组：各目录 `导入规则.json` 的「加载分组」字段唯一，收集器配置与它对账（R5）
