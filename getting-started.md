...
最省事的一条：**双击仓库根的 `panel.bat`**——它编面板与助手要跑的那几个工程、
起面板、等端口真应答了再把浏览器打开，**并把飞书助手一起起了**。停：双击 `panel-stop.bat`。
（编译时 DLL 被别的进程占着起不来，就用 `panel.bat /skip`，拿上次编好的产物起。）

助手为什么跟着一起起：机器人只在**长连接旁路活着的时候**收得到消息——
旁路没连上，发给它的话在飞书那头当场就丢，事后再起也补不回来。
「面板开着」不等于「机器人活着」这件事踩过一次，所以两者绑在一条命令里。
没配飞书密钥的机器会自动跳过助手并说明原因；不想起就 `panel.bat /nobot`。

想分开控制还是走脚本：

```powershell
./Tools/start.ps1               # 编译一次 → 影子拷贝 → 起面板 + 飞书助手
./Tools/start.ps1 -NoAssistant  # 只起面板
./Tools/start.ps1 -NoDashboard  # 只起助手
./Tools/stop.ps1                # 全停
```
