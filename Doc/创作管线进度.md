...
| P2 | 2 资产规格 | 资产规格数据三层合并 + 规格/落点缺省 + 资产规格门禁 | P2-1 | 未开始 |
| P2 | 3 生图 driver | `Bridges/comfyui/` 自述 + 配方骨架 + 依赖清单 + 能力对账（**真调用不可验**） | P2-1 | 未开始 |
| P2 | 4 选片与认领 | 选片卡片出站事件 + 专项认领入站（补 P1 批次 2 的已知缺口） | P2-1 | 未开始 |
| P2 | 5 审查与放行 | 放行策略数据 + 风险分级 + 审查包组装 | P1-2 | 未开始 |
...
到 P2 批次 1 为止**都不碰** `UnityProject/Assets/Game/Scripts/` 与 `Packages/com.hsgframe.*/Runtime/`，所以铁律 4 的分钟级 Unity 门禁**不适用**。
**P2 批次 2 起资产要往 `UnityProject/Assets/` 落**，那一批开始这一行必须改成「要跑 `gate-unity.ps1`」。
