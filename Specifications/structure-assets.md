...
- 跨模块通用的资产放**各自类型底下的 `Shared/` 门类**（`Art/Model/Shared/<模块>/`），判定门槛与代码同：被 ≥2 个模块引用就该在 Shared。
  **不许摆成 `Art/Shared/Model/`**（共享在前、类型在后）——那样同一个东西的模型与材质会走两条
  完全不同的路径，而分层规则的立身之本正是「把路径里的类型换掉就能找到另一种资产」。
  打包分组一个类型一条共享组前缀，最长前缀优先，所以它盖得过外层的业务组。
