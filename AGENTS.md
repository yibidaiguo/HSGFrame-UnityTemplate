<!-- 镜像文件：由 agent-sync.ps1 从同目录 CLAUDE.md 生成。改内容请改 CLAUDE.md，再重跑模板目录下的 Tools/AgentSync/agent-sync.ps1 -->
