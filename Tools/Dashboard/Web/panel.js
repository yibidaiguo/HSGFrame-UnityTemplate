...
var 工作台表 = {
    ...
    '美术': ['总览', '资产', '设计池', '需求池', '详情', '下游', '桥接包'],
    '程序': ['总览', '任务', '详情', '任务图', '门禁', '审查', '放行流水', '供给对账', '下游', '桥接包'],
...
function 渲染桥接包(数据) {
    ...
    var 文本 = '<div class="格 自适应" style="margin-bottom:16px;">' +
        ...
        '</div>';
    文本 += '<p class="弱 小字">这一页只报告与指路，不代你安装：它一个字都不会写进 manifest.json、editor-plugins.json 或 local.json。' +
        '能在这里点的只有「试跑一次」，其余照「下一步」那一列自己动手。</p>';
...
// 常用命令。这六条**逐条走面板真跑过**，命令名与参数名都对得上命令宿主——
// 原来的第五条写的是 spec.list，那个命令根本不存在（spec. 这一族至今一条命令都没有），
// 点下去只会得到「未找到命令」。写死一份清单就得跟真清单对过账，不然它就是六个陷阱。
var 速令表 = [
