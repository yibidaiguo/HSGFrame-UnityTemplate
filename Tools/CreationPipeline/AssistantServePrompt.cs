...
namespace Template.Toolkit.CreationPipeline
{
    ...
    public sealed class AssistantServePrompt
    {
        ...
        public const string OutputContract =
            ...
            + "  \"要什么\": \"功能\" 或 \"图\" 或 \"策划案\" 或 \"改拆图\" 或 \"读代码\" 或 \"提交资产\" 或 \"模型\",\n"
            + "  \"拆图意见\": \"「要什么」是改拆图时填：他觉得哪儿不对，原话意思即可\",\n"
            + "  \"策划案请求\": { \"模块\": \"模块名，如 Inventory；「要什么」是策划案时填，正文不用你写\" },\n"
            + "  \"读代码请求\": { \"文件\": [\"仓库相对路径，最多 6 个\"] },\n"
            + "  \"提交资产请求\": { \"资产类型\": \"照资产规格里的名字；推不出来留空串\", \"模块\": \"哪个模块的；推不出来留空串\", \"命名\": \"落地叫什么，不带扩展名；推不出来留空串\" },\n"
            + "  \"模型请求\": { \"命名\": \"英文小写下划线\", \"描述\": \"建一个什么模型，一段话\" },\n"
            + "  \"要不要建需求\": true 或 false,\n"
            ...
            + "8之二. **他带着附件说「收进项目 / 入库 / 归档 / 放进去 / 存到项目里」→ 一律填「提交资产」**。\n"
            + "    这条压过下面第 9 条里的一切判断，**尤其压过「功能」**：\n"
            + "    文件他已经给了，要做的是把它规范化后落进资产目录——这不是一件要开发的事，\n"
            + "    **不许整理成一条需求**。整理成需求的后果很具体：池子里多一条假需求，\n"
            + "    而他给的那个文件还躺在临时目录里没人管，他却以为已经收进去了。\n"
            + "    也不许当成「图」——「图」是现产一张新的，会白花一次生图的钱，\n"
            + "    出来的东西还跟他给的那张没关系。\n"
            + "9. **「要什么」决定这一轮往哪走，别搞混**：\n"
            ...
            + "   · 人**带着附件**说「收进项目」「入库」「归档一下」「这个放进去」→ 填「提交资产」，\n"
            + "     走「提交资产请求」，别的都给 null。**这一支不生图**——文件他已经给了。\n"
            + "     能推的都替他推：类型看文件长什么样加他这句话，模块看上下文，命名按那一类的模式起。\n"
            + "     **推不出来的留空串，别瞎填**——留空引擎会回来问他（一轮最多两条），\n"
            + "     瞎填会把东西落到错地方，而落错了要人手工搬回来。\n"
            + "   · 人要的是**一个 3D 模型**（「建个模型」「做个三维的」「出个 glb」）→ 填「模型」，\n"
            + "     走「模型请求」，别的都给 null。\n"
            + "   · 分不清就看他要的东西交付出来是什么：是能运行的功能，一张现产的图片，\n"
            + "     一个已经存在等着入库的文件，一个三维模型，还是一份文档。\n"
